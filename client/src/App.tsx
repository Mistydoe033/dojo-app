...
const GET_STARTER_PLAYER_MODELS = gql`
  query {
    dojoStarterPlayerModels(first: 10) {
      edges {
        cursor
        node {
          player_id
...
function hexToString(hex: string): string {
    try {
        return decodeURIComponent(
            hex
                .replace(/^0x/, '') // Remove the "0x" prefix
                .match(/.{1,2}/g) // Split into pairs of hex digits
                ?.map((byte) => String.fromCharCode(parseInt(byte, 16)))
                .join('') || ''
        );
    } catch {
        return hex; // Return the original value if decoding fails
    }
}
...
function App({ sdk }: { sdk: SDK<Schema> }) {
    ...
    // Use Apollo's useQuery to fetch player models
    const { loading, error, data } = useQuery(GET_STARTER_PLAYER_MODELS);

    const reversedPlayers = data
        ? [...data.dojoStarterPlayerModels.edges].reverse() // Create a new array and reverse it
        : [];
        ...
        <div className="bg-black min-h-screen w-full p-4 sm:p-8">
            <div className="max-w-7xl mx-auto">
                <div className="border border-white p-4 mb-4">
                    <h2 className="text-lg font-bold mb-4 text-white">Player List</h2>
                    <ul className="mb-4 space-y-4">
                        {loading && <li className="text-gray-500">Loading players...</li>}
                        {error && <li className="text-red-500">Error fetching players: {error.message}</li>}
                        {data &&
                            reversedPlayers.map(({ cursor, node }) => (
                                <li key={cursor} className="text-white">
                                    <strong>Player ID:</strong> {parseInt(node.player_id, 16)}
                                    <br />
                                    <strong>Username:</strong> {hexToString(node.username)}
                                </li>
                            ))}
                    </ul>
                </div>
